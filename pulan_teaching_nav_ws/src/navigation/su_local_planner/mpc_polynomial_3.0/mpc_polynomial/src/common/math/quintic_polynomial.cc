...
QuinticPolynomial::QuinticPolynomial(
    ...
    const double param)
    : QuinticPolynomial(start[0], start[1], start[2], end[0],
                        end[1],end[2], param) {}
...
QuinticPolynomial::QuinticPolynomial(
    const QuinticPolynomial& other) {
  param_ = other.param_;
  coef_ = other.coef_;
}

double QuinticPolynomial::Evaluate(const uint32_t order,
...
void QuinticPolynomial::SetParam(const double x0, const double dx0,
...
void QuinticPolynomial::IntegratedFromQuarticCurve(
    const QuinticPolynomial& other, const double init_value) {
