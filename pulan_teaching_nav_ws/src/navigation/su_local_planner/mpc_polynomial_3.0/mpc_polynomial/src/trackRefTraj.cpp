...
class FG_eval 
{
    public:
        ...
        std::vector<std::tuple<AD<double>, AD<double>, AD<double>>> tuples;
        ...
        AD<double> yaw = vars[_theta_start + i];
        // 在这里，你将不会推动数值进入tuples，而是推动变量本身
        // 当然，你需要修改tuples的类型，让它能够存储AD<double>类型
        tuples.push_back(std::make_tuple(x, y, yaw));
        }
      
            for (int i = 0; i < _mpc_steps; i++) 
            {
                cout << i << endl;
                cout << "_x_start" << CppAD::pow(vars[_cte_start + i] - _ref_cte, 2)<<endl;
            ...
            cout << "-----------------------------------------------" <<endl;
            cout << "cost_cte, cost_etheta, cost_velocity: " << cost_cte << ", " << cost_etheta  << ", " << cost_vel << endl;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) 
{
   ...
   //预测步长内预测的位姿
    for (const auto& t : fg_eval.tuples) {
    // 提取元组的每一部分，转为 double 类型
    double x = CppAD::Value(std::get<0>(t));
    double y = CppAD::Value(std::get<1>(t));
    double theta =CppAD::Value(std::get<2>(t));
    // 创建一个新的 double 类型元组
    std::tuple<double, double, double> new_tuple(x, y, theta);
    // 把新元组添加到目标 vector 中
    predict_points_tuples_.push_back(new_tuple);
    }
