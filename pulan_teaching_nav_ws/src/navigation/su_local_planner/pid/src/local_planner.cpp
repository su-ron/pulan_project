 ...
 */
void LocalPlanner::setFactor(double factor) { factor_ = factor; }
 ...
 */
bool LocalPlanner::shouldRotateToGoal(const geometry_msgs::PoseStamped& cur,
                                      const geometry_msgs::PoseStamped& goal) {
  double dx = cur.pose.position.x - goal.pose.position.x;
  double dy = cur.pose.position.y - goal.pose.position.y;
 ...
 * @return true if robot should perform rotation
 * 当方向误差阈值 (tolerance)
 * 存在且角度偏差(angle_to_path)大于这个阈值时，返回true，表示需要旋转。
 * 当角度偏差大于另一个预设的旋转阀值 (rotate_tol_)时，返回true，表示需要旋转。
 */
bool LocalPlanner::shouldRotateToPath(double angle_to_path, double tolerance) {
  return (tolerance && (angle_to_path > tolerance)) ||
         (angle_to_path > rotate_tol_);
}
 ...
 */
double LocalPlanner::linearRegularization(nav_msgs::Odometry& base_odometry,
                                          double v_d) {
 ...
 */
double LocalPlanner::angularRegularization(nav_msgs::Odometry& base_odometry,
                                           double w_d) {
