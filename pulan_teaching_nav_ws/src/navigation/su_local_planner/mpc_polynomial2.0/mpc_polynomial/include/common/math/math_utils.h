...
/**
 * @brief Check whether two points locate in the same plane split
 * by a straight line expressed as y = ax + b
 * @param coefficients The coefficients of linear function y = ax + b where
 * b = coefficients[0] and a = coefficients[1]
 * @param x0, y0 The priority point for comparison
 * @param x1, y1 The sub-priority point for comparison
 * @param dis The minimum distance of (x0, y0) to straight line
 */

 inline bool CheckInSamePlane(std::vector<double> coefficients,
  ...
  double val_0 = a * x0 - y0 + b;
  double val_1 = a * x1 - y1 + b;

  if (val_0 * val_1 > 0) {
    (*min_dis) = abs(a * x0 - y0 + b) / sqrt(a * a + 1);
    return true;
  } else {
    return false;
  }
