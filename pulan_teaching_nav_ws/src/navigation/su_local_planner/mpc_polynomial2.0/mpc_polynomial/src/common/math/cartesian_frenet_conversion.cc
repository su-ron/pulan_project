...
std::pair<double, double> CartesianFrenetConverter::CalculateCartesianXY(
    const double rtheta, const double& rx,
...
double CartesianFrenetConverter::CalculateSecondOrderLateralDerivative(
  ...
  if (std::isinf(res)) {
    printf("result is inf when calculate second order lateral \n");
  }
  return res;
}
