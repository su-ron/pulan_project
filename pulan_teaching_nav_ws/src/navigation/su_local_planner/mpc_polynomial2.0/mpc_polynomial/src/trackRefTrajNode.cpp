...
class MPCNode
{
    ...
    private:
        ...
        visualization_msgs::Marker points,forward_points,arrow, line_strip,forward_line_strip, goal_circle;
...
MPCNode::MPCNode()
{
    ...
    //初始化
    pathgenerator_.initparam();
    // 生成参考线
    pathgenerator_.GenerateRefline(ref_line_);
...
void MPCNode::initMarker() {
  ...
  =forward_line_strip.header.frame_id =goal_circle.header.frame_id =arrow.header.frame_id= "odom";

  points.ns =forward_points.ns = line_strip.ns =forward_line_strip.ns = goal_circle.ns=arrow.ns= "Markers";

  points.action = forward_points.action = line_strip.action = forward_line_strip.action = goal_circle.action =arrow.action
      visualization_msgs::Marker::ADD;
  points.pose.orientation.w = forward_points.pose.orientation.w = line_strip.pose.orientation.w 
  = forward_line_strip.pose.orientation.w =goal_circle.pose.orientation.w=arrow.pose.orientation.w = 1.0;
...
{     
    if(pathgenerator_.goal_received_&&!_goal_reached){ //received goal & goal not reached
    ROS_INFO("-------------Enter controlLoopCB2---------------------");    
    std::vector<Point> desired_path;
    nav_msgs::Path nav_path;
    // 生成的是一条基于Frenet坐标系的轨迹
    ROS_INFO(
        "pathgenerator_.start_pose_.x=%f,pathgenerator_.start_pose_.y=%f,"
        "pathgenerator_.start_pose_.yaw=%f,pathgenerator_.start_pose_.kappa=%f",
        pathgenerator_.start_pose_.x(), pathgenerator_.start_pose_.y(),
        pathgenerator_.start_pose_.yaw(), pathgenerator_.start_pose_.kappa());

    ROS_INFO(
        "pathgenerator_.target_pose_.x=%f,pathgenerator_.target_pose_.y=%f,"
        "pathgenerator_.target_pose_.yaw=%f,pathgenerator_.target_pose_.kappa=%f",
        pathgenerator_.target_pose_.x(), pathgenerator_.target_pose_.y(),
        pathgenerator_.target_pose_.yaw(), pathgenerator_.target_pose_.kappa());
    pathgenerator_.GenerateFrenetBasedSpline(pathgenerator_.start_pose_,
                                             pathgenerator_.target_pose_,
                                             ref_line_, &desired_path);
    ROS_INFO("desired_path.size()=%zu",desired_path.size());


    pathgenerator_.converttrajToNavPath(desired_path, nav_path);
