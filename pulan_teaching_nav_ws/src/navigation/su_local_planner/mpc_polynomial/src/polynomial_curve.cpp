...
void Polynomial::goalCB(const geometry_msgs::PoseStamped::ConstPtr& goalMsg)
{
  ...
  goal_yaw_=tf::getYaw(goalMsg->pose.orientation);
  ...
  midpointpose_base_.pose.position.x = goalpose_base_.pose.position.x/4;
  ...
  pointpose_y_2_base_.pose.position.x = goalpose_base_.pose.position.x/2;
  pointpose_y_2_base_.pose.position.y = goalpose_base_.pose.position.y;
...
bool Polynomial::run(const Points2d points, Points2d& path,nav_msgs::Path&desired_path)
{
    ...
    poses.emplace_back(points.back().first, points.back().second, goal_yaw_);
...
bool Polynomial::run(const Poses2d points, Points2d& path,nav_msgs::Path&desired_path)
{
    ...
    //启发式生成速度和加速度约束
    //创建了一个 std::vector<double> 类型的向量 v，其长度为 points.size()，并且初始化每个元素的值为 0.1。
    std::vector<double> v(points.size(), 0.1);
    //起点终点速度设置为0
    v[0] = 0.0;
    v[points.size() - 1] = 0;
...
int main(int argc, char **argv)
{
    ...
    if(polynomial.initpose_received_&&polynomial.goal_received_)
    {
      ...
      Points2d points = {{polynomial.initpose_x_, polynomial.initpose_y_},
                         {polynomial.midpointpose_map_.pose.position.x,polynomial.midpointpose_map_.pose.position.y},
                         {polynomial.pointpose_y_2_map_.pose.position.x,polynomial.pointpose_y_2_map_.pose.position.y},
