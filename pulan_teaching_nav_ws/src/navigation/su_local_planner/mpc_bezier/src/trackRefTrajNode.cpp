...
void MPCNode::desiredPathCB(const nav_msgs::Path::ConstPtr& totalPathMsg)
{
    ...
    //ROS_INFO("Received path message with %zu poses", totalPathMsg->poses.size());
     ...
     //ROS_INFO("totalPathMsg frame ID: %s", totalPathMsg->header.frame_id.c_str());
    for (size_t i = 0; i < totalPathMsg->poses.size(); i++) {
        // 获取每个姿态的位置和姿态信息
        double x = totalPathMsg->poses[i].pose.position.x;
        double y = totalPathMsg->poses[i].pose.position.y;
        double yaw = tf::getYaw(totalPathMsg->poses[i].pose.orientation);
        
        // 打印每个姿态的信息
       // ROS_INFO("Pose %zu: x=%f, y=%f, yaw=%f", i, x, y, yaw);
    }
